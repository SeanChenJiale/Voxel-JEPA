    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        for itr in range(ipe):
            ...
            assert len(masks_enc) == len(masks_pred), \
                ...
                # logger.info(f"len of udata[0] {len(udata[0])}")
                # logger.info(f"shape of udata[0] {[udat.shape for udat in udata[0]]}")
                # logger.info(f"len of udata[1] {len(udata[1])}")
                # logger.info(f"print type of udata[1] {type(udata[1])}")
                # logger.info(f"shape of udata[1] {udata[1].shape}")
                # logger.info(f"udata[1] {udata[1]}")
                # logger.info(f"len of udata[2] {len(udata[2])}")
            ...
            ########## debug phase #####
            # if debug:
            #     print(f"Clips shape: {clips.shape}, Clips content: {clips}")
            #     logger.info(f"DEBUG shape of each mask in mask_enc AFTER load_clips: {[mask.shape for mask in masks_enc]}")
                ...
                def loss_fn(z, h):
                    ...
                    for zi, hi in zip(z, h):
                        ...
                        if save_mask:
                            ...
                            torch.save(hi, os.path.join(epoch_dir_path,f"iter{itr}_hi_mask{count}.pt"))
                            count+=1
