...
def main(args_eval, plotter, resume_preempt=False, debug=False):
    ...
    for name, param in encoder.named_parameters():
        if not param.requires_grad:
            print(f"{name} with shape {param.shape} is frozen")
...
def run_one_epoch(
    ...
    for itr, data in enumerate(data_loader):
        ...
        # print(f"Iter {itr}: Loss={loss.item()}, Outputs shape={outputs.shape}, Outputs sample={outputs[:2]}")
        # print(f"Labels sample={labels[:2]}")
        # print(f"Loss requires grad: {loss.requires_grad}")
        # import pdb; pdb.set_trace()
        # import copy
        # encoder_weights_before = [p.clone().detach() for p in encoder.parameters()]
        if training:
            if use_bfloat16:
                scaler.scale(loss).backward()
                grad_norm = torch.nn.utils.clip_grad_norm_(list(encoder.parameters()) + list(classifier.parameters()), 10.0)
                ...
                loss.backward()
                # import pdb; pdb.set_trace()
                torch.nn.utils.clip_grad_norm_(list(encoder.parameters()) + list(classifier.parameters()), 10.0)
