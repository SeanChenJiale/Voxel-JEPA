...
class AttentiveRegressor(nn.Module):
    ...
    def __init__(
        ...
        init_std=0.02,
        qkv_bias=True,
        output_dim=1,  # Number of regression outputs
        complete_block=True,
    ):
        super().__init__()
        self.pooler = AttentivePooler(
        ...
        self.linear = nn.Linear(embed_dim, output_dim, bias=True)  # Regression head

    def forward(self, x):
        x = self.pooler(x).squeeze(1)
        x = self.linear(x)  # Outputs raw regression values
        return x