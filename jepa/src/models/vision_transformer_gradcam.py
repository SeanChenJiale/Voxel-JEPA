...
class ViTGradCAMHook:
    ...
    def save_activation(self, module, input, output):
        print("[GradCAM] Forward hook called. Activation shape:", output.shape)
        self.activations = output

    def save_gradient(self, grad):
        print("[GradCAM] Backward hook called. Gradient shape:", grad.shape)
        self.gradients = grad
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x, masks=None, gradcam_hook=None):
        """
        ...
        for i, blk in enumerate(self.blocks):
            ...
            # Register forward hook on the last block if gradcam_hook is provided
            if gradcam_hook is not None and i == len(self.blocks) - 1:
                # Register forward hook to save activations
                x.register_hook(gradcam_hook.save_gradient)
                gradcam_hook.activations = x.detach()
