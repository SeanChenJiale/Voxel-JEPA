...
class _MaskGenerator(object):
    ...
    def _sample_block_mask(self):
        """
        Create a mask that obscures half of the image.
        Randomly selects one of the following regions to obscure:
        - Full top half
        - Full bottom half
        - Full left half
        - Full right half
        ...
        # Randomly choose one of the four regions to obscure
        region = torch.randint(0, 4, (1,)).item()

        if region == 0:  # Obscure the top half
            mask[:, :self.height // 2, :] = 0
        elif region == 1:  # Obscure the bottom half
            mask[:, self.height // 2:, :] = 0
        elif region == 2:  # Obscure the left half
            mask[:, :, :self.width // 2] = 0
        elif region == 3:  # Obscure the right half
            mask[:, :, self.width // 2:] = 0
