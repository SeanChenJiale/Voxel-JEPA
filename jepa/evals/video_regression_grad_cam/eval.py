...
def run_one_epoch(
    ...
    # Suppose outputs is [batch_size, n_outputs]
    target_idx = 0  # the output dimension you want Grad-CAM for
    print(len(outputs[0]))
    print([d.size() for d in outputs[0]])
    score = outputs[0][0][0, target_idx]  # target_idx-th regression output
    print(score)
    ...
    if not saved and hook is not None and hasattr(hook, 'activations') and hook.activations is not None and hasattr(hook, 'gradients') and hook.gradients is not None:
        ...
        if isinstance(outputs, list):
            if isinstance(outputs[0], list):
                # outputs[segment][view][sample]
                pred_value = outputs[0][0][sample_idx].item()
            else:
                # outputs[segment][sample]
                pred_value = outputs[0][sample_idx].item()
        else:
            # outputs[sample]
            pred_value = outputs[sample_idx].item()
        torch.save({
            'input': input_sample,
