...
def run_one_epoch(
    ...
    for itr, data in enumerate(data_loader):
        ...
        # Get the final outputs for accuracy calculation
        if attend_across_segments:
            final_outputs = sum([F.softmax(o, dim=1) for o in outputs]) / len(outputs)
        else:
            final_outputs = sum([sum([F.softmax(ost, dim=1) for ost in os]) for os in outputs]) / len(outputs) / len(outputs[0])
        
        top1_acc = 100. * final_outputs.max(dim=1).indices.eq(labels).sum() / batch_size
        top1_acc = float(AllReduce.apply(top1_acc))
        top1_meter.update(top1_acc)

        # Get predictions and true labels for class-conditional analysis
        predicted_classes = final_outputs.argmax(dim=1)
        true_classes = labels

        # Perform backward pass for each sample to get class-conditional gradients
        for sample_idx in range(batch_size):
            pred_class = predicted_classes[sample_idx].item()
            true_class = true_classes[sample_idx].item()
            
            # Choose what you want to analyze:
            # Option A: Predicted class (what model thinks it sees)
            target_class = pred_class
            # Option B: True class (what it should see - uncomment below to use)
            # target_class = true_class
            
            # Create one-hot vector for target class
            one_hot = torch.zeros_like(final_outputs)
            one_hot[sample_idx, target_class] = 1
            ...
            final_outputs.backward(gradient=one_hot, retain_graph=True)
            
            # Store gradients and activations by class
            if hook is not None and hasattr(hook, 'activations') and hook.activations is not None and hasattr(hook, 'gradients') and hook.gradients is not None:
                ...
                # Store sample data
                sample_activations = hook.activations[sample_idx].detach().cpu()
                sample_gradients = hook.gradients[sample_idx].detach().cpu()
    ...
    if class_gradients:
        print(f"[GradCAM] Computing class-conditional average Grad-CAM")
        
        for class_id in class_gradients.keys():
            if len(class_gradients[class_id]) > 0:
                print(f"[GradCAM] Processing class {class_id} with {class_counts[class_id]} samples")
                
                # Stack all activations and gradients for this class
                stacked_activations = torch.stack(class_activations[class_id])
                stacked_gradients = torch.stack(class_gradients[class_id])
                
                # Compute average for this class
                avg_activations = torch.mean(stacked_activations, dim=0)
                avg_gradients = torch.mean(stacked_gradients, dim=0)
                
                # Save class-specific Grad-CAM
                class_save_path = os.path.join(gradcam_path, f'gradcam_class_{class_id}.pt')
                torch.save({
                    'avg_activations': avg_activations,
                    'avg_gradients': avg_gradients,
                    'input': class_inputs[class_id],
                    'class_id': class_id,
                ...
                # # Generate and save the class-specific Grad-CAM visualization
                # save_gradcam(class_save_path,class_id)
                
                # Read and visualize the tensor
                read_tensor(class_save_path,class_id)
